...
public class UmaMetadataWS {
    ...
    @Operation(
    		description = "Provides configuration data as json document. It contains options and endpoints supported by the authorization server.",
    				responses =  {
    	    	    		@ApiResponse(description = "Reponse object containing the revoke request status", content = @Content(schema = @Schema(implementation = UmaMetadata.class)))
    	    	    }
            /*response = UmaMetadata.class*/
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "500", description = "Failed to build Uma configuration json object.")
    })
