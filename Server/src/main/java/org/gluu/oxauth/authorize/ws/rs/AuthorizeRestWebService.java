 ...
 */
@Schema(defaultValue = "/", description = "The Authorization Endpoint performs Authentication of the End-User. This is done by sending the User Agent to the Authorization Server's Authorization Endpoint for Authentication and Authorization, using request parameters defined by OAuth 2.0 and additional parameters and parameter values defined by OpenID Connect.")
public interface AuthorizeRestWebService {
    ...
    @Operation(
            description = "Performs authorization.",
            summary = "The Authorization Endpoint performs Authentication of the End-User.",
    		responses =  {
    	    		@ApiResponse(description = "Reponse object containing the HTTP GET authorization request status details.", content = @Content(schema = @Schema(implementation = Response.class), mediaType="JSON"))
    	    }
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "302", description = "interaction_required\n" +
                    "    The Authorization Server requires End-User interaction of some form to proceed. This error MAY be returned when the prompt parameter value in the Authentication Request is none, but the Authentication Request cannot be completed without displaying a user interface for End-User interaction. "),
            @ApiResponse(responseCode = "302", description = "login_required\n" +
                    "    The Authorization Server requires End-User authentication. This error MAY be returned when the prompt parameter value in the Authentication Request is none, but the Authentication Request cannot be completed without displaying a user interface for End-User authentication. "),
            @ApiResponse(responseCode = "302", description = "account_selection_required\n" +
                    "    The End-User is REQUIRED to select a session at the Authorization Server. The End-User MAY be authenticated at the Authorization Server with different associated accounts, but the End-User did not select a session. This error MAY be returned when the prompt parameter value in the Authentication Request is none, but the Authentication Request cannot be completed without displaying a user interface to prompt for a session to use. "),
            @ApiResponse(responseCode = "302", description = "consent_required\n" +
                    "    The Authorization Server requires End-User consent. This error MAY be returned when the prompt parameter value in the Authentication Request is none, but the Authentication Request cannot be completed without displaying a user interface for End-User consent. "),
            @ApiResponse(responseCode = "302", description = "invalid_request_uri\n" +
                    "    The request_uri in the Authorization Request returns an error or contains invalid data. "),
            @ApiResponse(responseCode = "302", description = "invalid_request_object\n" +
                    "    The request parameter contains an invalid Request Object. "),
            @ApiResponse(responseCode = "302", description = "request_not_supported\n" +
                    "    The OP does not support use of the request parameter"),
            @ApiResponse(responseCode = "302", description = "request_uri_not_supported\n" +
                    "    The OP does not support use of the request_uri parameter"),
            @ApiResponse(responseCode = "302", description = "registration_not_supported\n" +
                    "    The OP does not support use of the registration parameter")
    })
    Response requestAuthorizationGet(
            @QueryParam("scope")
            @Parameter(description = "OpenID Connect requests MUST contain the openid scope value. If the openid scope value is not present, the behavior is entirely unspecified. Other scope values MAY be present. Scope values used that are not understood by an implementation SHOULD be ignored.", required = true)
                    String scope,
            @QueryParam("response_type")
            @Parameter(description = "OAuth 2.0 Response Type value that determines the authorization processing flow to be used, including what parameters are returned from the endpoints used. When using the Authorization Code Flow, this value is code. ", required = true)
                    String responseType,
            @QueryParam("client_id")
            @Parameter(description = "OAuth 2.0 Client Identifier valid at the Authorization Server.", required = true)
                    String clientId,
            @QueryParam("redirect_uri")
            @Parameter(description = "Redirection URI to which the response will be sent. This URI MUST exactly match one of the Redirection URI values for the Client pre-registered at the OpenID Provider", required = true)
                    String redirectUri,
            @QueryParam("state")
            @Parameter(description = "Opaque value used to maintain state between the request and the callback. Typically, Cross-Site Request Forgery (CSRF, XSRF) mitigation is done by cryptographically binding the value of this parameter with a browser cookie. ", required = false)
                    String state,
            @QueryParam("response_mode")
            @Parameter(description = "Informs the Authorization Server of the mechanism to be used for returning parameters from the Authorization Endpoint. This use of this parameter is NOT RECOMMENDED when the Response Mode that would be requested is the default mode specified for the Response Type. ", required = false)
                    String responseMode,
            @QueryParam("nonce")
            @Parameter(description = "String value used to associate a Client session with an ID Token, and to mitigate replay attacks. The value is passed through unmodified from the Authorization Request to the ID Token. Sufficient entropy MUST be present in the nonce values used to prevent attackers from guessing values.", required = false)
                    String nonce,
            @QueryParam("display")
            @Parameter(description = "ASCII string value that specifies how the Authorization Server displays the authentication and consent user interface pages to the End-User. The defined values are: page, popup, touch, wap", required = false)
                    String display,
            @QueryParam("prompt")
            @Parameter(description = "Space delimited, case sensitive list of ASCII string values that specifies whether the Authorization Server prompts the End-User for reauthentication and consent. The defined values are: none, login, consent, select_account", required = false)
                    String prompt,
            @QueryParam("max_age")
            ...
            @Parameter(description = "AMR Values", required = false)
                    String amrValues,
            @QueryParam("request")
            @Parameter(description = "This parameter enables OpenID Connect requests to be passed in a single, self-contained parameter and to be optionally signed and/or encrypted. The parameter value is a Request Object value, as specified in Section 6.1. It represents the request as a JWT whose Claims are the request parameters.", required = false)
                    String request,
            @QueryParam("request_uri")
            @Parameter(description = "This parameter enables OpenID Connect requests to be passed by reference, rather than by value. The request_uri value is a URL using the https scheme referencing a resource containing a Request Object value, which is a JWT containing the request parameters. ", required = false)
                    String requestUri,
            @QueryParam("request_session_id")
            @Parameter(description = "Request session id", required = false)
                    String requestSessionId,
            @QueryParam("session_id")
            @Parameter(description = "Session id of this call", required = false)
                    String sessionId,
            @QueryParam("origin_headers")
            @Parameter(description = "Origin headers. Used in custom workflows.", required = false)
                    String originHeaders,
            @QueryParam("code_challenge")
            @Parameter(description = "PKCE code challenge.", required = false)
                    String codeChallenge,
            @QueryParam("code_challenge_method")
            @Parameter(description = "PKCE code challenge method.", required = false)
                    String codeChallengeMethod,
            @QueryParam(AuthorizeRequestParam.CUSTOM_RESPONSE_HEADERS)
            @Parameter(description = "Custom Response Headers.", required = false)
                    String customResponseHeaders,
            @QueryParam("claims")
            @Parameter(description = "Requested Claims.", required = false)
                    String claims,
            @QueryParam("auth_req_id")
            @Parameter(description = "CIBA authentication request Id.", required = false)
                    String authReqId,
    ...
    @Operation(
            ...
            summary = "The Authorization Endpoint performs Authentication of the End-User.",
    		responses =  {
    	    		@ApiResponse(description = "Reponse object containing the authentication request status.", content = @Content(schema = @Schema(implementation = Response.class), mediaType="JSON"))
    	    }
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "302", description = "interaction_required\n" +
