 ...
 */
@Schema(defaultValue = "/", description = "The UserInfo Endpoint is an OAuth 2.0 Protected Resource that returns Claims about the authenticated End-User. To obtain the requested Claims about the End-User, the Client makes a request to the UserInfo Endpoint using an Access Token obtained through OpenID Connect Authentication. These Claims are normally represented by a JSON object that contains a collection of name and value pairs for the Claims. ")
public interface UserInfoRestWebService {
    ...
    @Operation(
            ...
            summary = "The Access Token obtained from an OpenID Connect Authentication Request is sent as a Bearer Token.",
    		responses =  {
    	    		@ApiResponse(description = "Reponse object containing the authenticated End-User details", content = @Content(schema = @Schema(implementation = Response.class), mediaType="JSON"))
    	    }
   )
    @ApiResponses(value = {
    ...
    Response requestUserInfoGet(
            ...
            @Parameter(description = "OAuth 2.0 Access Token.", required = true)
                    String accessToken,
