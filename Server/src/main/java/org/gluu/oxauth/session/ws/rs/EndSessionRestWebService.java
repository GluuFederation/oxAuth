 ...
 */
@Schema(defaultValue = "/", description = "End Session Endpoint - URL at the OP to which an RP can perform a redirect to request that the End-User be logged out at the OP")
public interface EndSessionRestWebService {
    ...
    @Operation(
    		description = "End current Connect session.",
            summary = "End current Connect session.",
    		responses =  {
    	    		@ApiResponse(description = "Reponse object containing the logout request status", content = @Content(schema = @Schema(implementation = Response.class), mediaType="JSON"))
    	    }
    )
    @ApiResponses(value = {
