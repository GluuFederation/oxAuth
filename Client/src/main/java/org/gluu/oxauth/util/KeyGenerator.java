...
import org.gluu.oxauth.model.crypto.OxAuthFIPSCryptoProvider;
 ...
 * org.gluu.oxauth.util.KeyGenerator -h
 * <p/>
 * KeyGenerator -sig_keys RS256 RS384 RS512 ES256 ES384 ES512 PS256 PS384 PS512
 * -enc_keys RSA_OAEP RSA1_5 -keystore /Users/JAVIER/tmp/mykeystore.jks
 * -keypasswd secret -dnname "CN=oxAuth CA Certificates" -expiration 365
 * <p/>
...
public class KeyGenerator {
	...
	public static class Cli {
		...
		public void parse() {
			...
			try {
				...
				if (cmd.hasOption(OXELEVEN_ACCESS_TOKEN) && cmd.hasOption(OXELEVEN_GENERATE_KEY_ENDPOINT)) {
					...
					try {
						OxElevenCryptoProvider cryptoProvider = new OxElevenCryptoProvider(generateKeyEndpoint, null,
								null, null, accessToken);

						generateKeys(cryptoProvider, signatureAlgorithms, encryptionAlgorithms, expiration,
								expiration_hours, keyLength);
					} catch (Exception e) {
		...
		private void generateKeys(AbstractCryptoProvider cryptoProvider, List<Algorithm> signatureAlgorithms,
				List<Algorithm> encryptionAlgorithms, int expiration, int expiration_hours, int keyLength)
				throws Exception {
